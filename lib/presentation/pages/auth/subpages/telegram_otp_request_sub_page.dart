...
@RoutePage()
class TelegramOtpRequestSubPage extends StatefulWidget {
  const TelegramOtpRequestSubPage({super.key, this.email = ''});

  final String email;

  @override
  State<TelegramOtpRequestSubPage> createState() =>
      _TelegramOtpRequestSubPageState();
}

class _TelegramOtpRequestSubPageState
    extends State<TelegramOtpRequestSubPage> {
  late var _email = widget.email;

  @override
  Widget build(BuildContext context) =>
      BlocListener<TelegramOtpLoginCubit, TelegramOtpLoginState>(
        listenWhen: (p, c) => p.request != c.request,
        listener: (context, state) {
          if (state.request case LoadedStateData()) {
            context.pushRoute(const TelegramOtpVerifySubRoute());
          }
        },
        child: AppScaffold(
          title: 'Login with Telegram code',
          topSafeArea: false,
          body: AppListBody(
            children: [
              AppTextField(
              ...
              AppButton(
                onTap: () => context.read<TelegramOtpLoginCubit>().requestOtp(
                  email: _email,
                ),
